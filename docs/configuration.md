...
# =============================================================================
# matrix: run concurrent models with capacity-based swapping
# =============================================================================
#
# Note:
...
#
# The preferred matrix mode uses a simple capacity budget. Define how much
# capacity your system has, then define how much each model consumes. Units are
# arbitrary: use GB of VRAM, total RAM, weighted VRAM, or any conservative number
# that matches your hardware. If the numbers are wrong, the backend may OOM just
# like it would with an incorrect manual compatibility matrix.
# A model whose memory is larger than capacity is treated as capacity-sized, so
# it will run alone and evict all other running models when requested.
#
# Solver behavior:
#   1. Request arrives for model X
#   2. If X is already running, forward immediately. Done.
#   3. If X fits with the currently running models, start X. Done.
#   4. If X does not fit, evict running models until it fits.
#
# Eviction strategy:
#   cost: evict the lowest total evictCost needed to fit the request.
#         If evictCost is omitted, memory is used as the cost.
#   lru:  evict least-recently-used running models until the request fits.
#
# Example per-model fields:
#
# models:
#   gemma-model:
#     cmd: ...
...
#     cmd: ...
#     memory: 12
#     evictCost: 50 # slow cold start, avoid evicting when possible
#
matrix:
  capacity: 128
  strategy: cost # cost (default) or lru
  # Legacy/advanced set-DSL mode is still supported by defining vars + sets
  # instead of capacity.
